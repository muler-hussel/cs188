...
def constructBayesNet(gameState: hunters.GameState):
    ...
    position = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for ev in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, ev)
            if len(joinedFactor.unconditionedVariables()) > 1:
                marginalizedJoint = eliminate(joinedFactor, ev)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newNoise = DiscreteDistribution()
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for p in newPosDist.keys():
                newNoise[p] += newPosDist[p] * self.beliefs[oldPos]
        self.beliefs = newNoise
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for _ in range(self.numParticles):
            idx = random.randint(0, len(self.legalPositions) - 1)
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            if particle in belief:
                belief[particle] += 1 / total
            else:
                belief[particle] = 1 / total
        return DiscreteDistribution(belief)
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for gp in self.particles:
            newProb[gp] += self.getObservationProb(observation, pacmanPosition, gp, jailPosition)
        
        if newProb.total() == 0:
            self.initializeUniformly(gameState)
        else:
            newProb.normalize()
            self.particles = [newProb.sample() for _ in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            if oldPos not in newPro:
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                newPro[oldPos] = newPosDist
                self.particles[count] = newPro[oldPos].sample()
            else:
                self.particles[count] = newPro[oldPos].sample()
            count += 1
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        positionTuples = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(positionTuples)
        self.particles = positionTuples
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        newProb = self.getBeliefDistribution()
